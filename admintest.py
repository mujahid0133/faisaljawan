...
# --- Batch HTML Export Action ---


def export_invoices_html(modeladmin, request, queryset):
    buffer = io.BytesIO()
    with zipfile.ZipFile(buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
        for inv in queryset:
            # Render HTML for each invoice (mimic the individual invoice view)
            context = {
                'invoice': inv,
                'items': inv.items.all(),
                'subtotal': inv.total_excl_tax,
                'total_tax': inv.total_tax,
                'grand_total': inv.total_incl_tax,
                'special_case': '',
            }
            html = render_to_string('index.html', context)
            filename = f"{inv.invoice_no}.html"
            zip_file.writestr(filename, html)
    buffer.seek(0)
    response = HttpResponse(buffer, content_type='application/zip')
    response['Content-Disposition'] = 'attachment; filename="invoices_html.zip"'
    return response


export_invoices_html.short_description = "Export selected invoices as HTML (zip)"
...
class InvoiceAdmin(admin.ModelAdmin):
    ...
    actions = [mark_as_paid, mark_as_unpaid,
               export_invoices_csv, export_invoices_html]
